...
(() => {
  ...
  function spawnLyricWord(container, text, ticker) {
    ...
    const isCalm = isCrustacean || isAdhoc;
    const chaos = !isCalm && Boolean(ticker && ticker.chaos);
    const speed = LYRIC_TEMPO_SPEED[(ticker && ticker.tempo) || 'normal'] || 1;
    const variants = isCrustacean ? CRUSTACEAN_LYRIC_VARIANTS : isAdhoc ? ADHOC_LYRIC_VARIANTS : LYRIC_VARIANTS;
    const fonts = isCrustacean ? CRUSTACEAN_LYRIC_FONTS : isAdhoc ? ADHOC_LYRIC_FONTS : (chaos ? LYRIC_FONTS_CHAOS : LYRIC_FONTS);
    const variant = variants[Math.floor(Math.random() * variants.length)];
    const isBg = variant === 'v-bg';
    const isFly = variant === 'v-fly';
    const isGlitch = !isCalm && Math.random() < (chaos ? 0.6 : 0.38);
    const size = isCalm ? 11 + Math.random() * 24 : isBg ? 50 + Math.random() * 100 : 8 + Math.random() * 46;
    const font = fonts[Math.floor(Math.random() * fonts.length)];
    const top = Math.random() * (isCalm ? 88 : 84);
    const left = isFly ? 0 : Math.random() * (isCalm ? 72 : 62);
    const rotSpread = (isCalm ? 4 : 18) + (chaos ? 14 : 0);
    const rot = (Math.random() * rotSpread - rotSpread / 2).toFixed(1);
    const dx = (Math.random() * (isCalm ? 30 : 70) - (isCalm ? 15 : 35)).toFixed(0);
    const dy = (Math.random() * (isCalm ? 24 : 70) - (isCalm ? 12 : 35)).toFixed(0);
    const flyFrom = Math.random() < 0.5 ? '-20%' : '118%';
    const flyTo = flyFrom === '-20%' ? '118%' : '-20%';
    const dur = ((isCalm ? 14 + Math.random() * 12 : isFly ? 3.5 + Math.random() * 2.5 : isBg ? 8 + Math.random() * 6 : 5 + Math.random() * 4) / speed).toFixed(2);
    const delay = ((Math.random() * (isCalm ? 4 : 1.4)) / speed).toFixed(2);
    ...
    span.style.cssText = `font-size:${size.toFixed(0)}px;font-family:${font};top:${top.toFixed(1)}%;left:${left.toFixed(1)}%;--rot:${rot}deg;--dx:${dx}px;--dy:${dy}px;--fly-from:${flyFrom};--fly-to:${flyTo};--alpha:${(isCalm ? 0.2 + Math.random() * 0.48 : 1).toFixed(2)};--depth:${(Math.random() * 18 - 9).toFixed(0)}px;animation-duration:${dur}s;animation-delay:${delay}s;animation-timing-function:${timing}`;
